...
class _MainScreenState extends State<MainScreen> with TickerProviderStateMixin {
  ...
  void saveRideRequest(){
    ...
    rideStreamSubscription = rideRequestRef?.onValue.listen((event) async {
      ...
      if(data['driver_location'] !=null){
         ...
         double driverLong = double.parse(data['driver_location']['longitude'].toString());
         ...
         if(statusRide == "Ended"){
           if(data['fares'] !=null){
             ...
             var res = await showDialog(context: context, builder: (ctx){
               return CollectFareDailog(paymentMethod: "cash", fareAmount: fare);
             });
             String driverId="";
             if(res == "close"){
               ...
               Get.to(()=>RatingScreen(driverId: driverId,));
  ...
  void updateRideTimeToDropLoc(LatLng driverCurrentLocation) async{
    if(isRequestingPositionDetails == false){
      isRequestingPositionDetails = true;
      var dropOff = Provider.of<AppData>(context,listen: false).dropOffLocation;
      var dropOffLatln = LatLng(dropOff.latitude, dropOff.latitude);

      var details = await AssistantMethods.obtainPlaceDirectionDetails(driverCurrentLocation, dropOffLatln);
      if(details == null){
        return;
      }
      setState(() {
                            ...
                            child: Row(
                              children: [
                                ...
                                Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    Text("Bike",style: TextStyle(fontSize: 18.0,fontFamily: "Brand-Bold"),),
                                    Text(
                                      ((tripDirectionDetails !=null?'${tripDirectionDetails?.distanceText}':'')),style: TextStyle(fontSize: 18.0,color: Colors.grey),),
                                  ],
                                ),
                                Expanded(child: Container()),
                                Text((tripDirectionDetails !=null?'PKR-${(AssistantMethods.calculateFares(tripDirectionDetails!))/2}':''),style: TextStyle(fontFamily: "Brand-Bold"),),
                              ],
                            ),
                          ),
                        ),
                      ...
                      GestureDetector(
                        ...
                        child: Container(
                          ...
                          child: Padding(padding: EdgeInsets.symmetric(horizontal: 16.0),
                            child: Row(
                              children: [
                                Image.asset("images/buraqSuper.png",height: 70.0,width: 80.0,),
                                SizedBox(width: 16.0,),
                                Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    Text("Buraq Super",style: TextStyle(fontSize: 18.0,fontFamily: "Brand-Bold"),),
                                    Text(
                                      ((tripDirectionDetails !=null?'${tripDirectionDetails?.distanceText}':'')),style: TextStyle(fontSize: 18.0,color: Colors.grey),),
                                  ],
                                ),
                                Expanded(child: Container()),
                                Text((tripDirectionDetails !=null?'PKR-${AssistantMethods.calculateFares(tripDirectionDetails!)}':''),style: TextStyle(fontFamily: "Brand-Bold"),),
                              ],
                            ),
                          ),
                        ),
                      ...
                      GestureDetector(
                        ...
                        child: Container(
                          ...
                          child: Padding(padding: EdgeInsets.symmetric(horizontal: 16.0),
                            child: Row(
                              children: [
                                Image.asset("images/buraqUltra.png",height: 70.0,width: 80.0,),
                                SizedBox(width: 16.0,),
                                Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    Text("Buraq Ultra",style: TextStyle(fontSize: 18.0,fontFamily: "Brand-Bold"),),
                                    Text(
                                      ((tripDirectionDetails !=null?'${tripDirectionDetails?.distanceText}':'')),style: TextStyle(fontSize: 18.0,color: Colors.grey),),
                                  ],
                                ),
                ...
                child:  Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Divider(height: 2.0,thickness: 2.0,),
                      SizedBox(height: 22.0,),
                      Text(car_details,style: TextStyle(color: Colors.grey),),
                            ...
                            child: MaterialButton(
                              ...
                              child: Padding(padding: EdgeInsets.fromLTRB(5.0, 0.0, 5.0, 0.0),
                                child: Row(
                                  mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                                  children: const [
                                    Text('Call',style: TextStyle(fontSize: 20.0,fontWeight: FontWeight.bold,color: Colors.white),),
                            ...
                            child: MaterialButton(
                              ...
                              child: Padding(padding: EdgeInsets.fromLTRB(5.0, 0.0, 5.0, 0.0),
                                child: Row(
                                  mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                                  children: const [
                                    Text('Message',style: TextStyle(fontSize: 20.0,fontWeight: FontWeight.bold,color: Colors.white),),
                                    Icon(Icons.sms,color: Colors.white,size: 26.0,),
                                  ],
                                ),
                              ),
                            ),
  ...
  Future<void> getPlaceDirection() async{
    ...
    print("${initialPos.latitude}+${initialPos.longititue}");
    print("${finalPos.latitude}+${finalPos.longititue}");
    ...
    showDialog(context: context, builder: (BuildContext context)=>ProgressDialod(message: "Please wait...."));

    var details = await AssistantMethods.obtainPlaceDirectionDetails(pickUpLatlan, dropoffLatlan);
    ...
    polylineSet.clear();
    setState(() {
      Polyline polyline = Polyline(polylineId: PolylineId("PolylineID"),color: Colors.pink,jointType: JointType.round,points: pLineCoordinates,width: 5,startCap: Cap.roundCap,endCap: Cap.roundCap,geodesic: true);
      polylineSet.add(polyline);
    });
    LatLngBounds latLngBounds;
    if(pickUpLatlan.latitude>dropoffLatlan.latitude && pickUpLatlan.longitude>dropoffLatlan.longitude){
      latLngBounds = LatLngBounds(southwest: dropoffLatlan, northeast: pickUpLatlan);
    }else if(pickUpLatlan.latitude>dropoffLatlan.latitude){
      latLngBounds = LatLngBounds(southwest: LatLng(dropoffLatlan.latitude, pickUpLatlan.longitude), northeast: LatLng(pickUpLatlan.latitude, dropoffLatlan.longitude));
    }else if(pickUpLatlan.longitude>dropoffLatlan.longitude){
      latLngBounds = LatLngBounds(southwest: LatLng(pickUpLatlan.latitude, dropoffLatlan.longitude), northeast: LatLng(dropoffLatlan.latitude, pickUpLatlan.longitude));
    }else{
      latLngBounds = LatLngBounds(southwest: pickUpLatlan, northeast: dropoffLatlan);
    }

    newGoogleMapController.animateCamera(CameraUpdate.newLatLngBounds(latLngBounds, 70));
    Marker pickUpLocationMarker = Marker(markerId: MarkerId("Pick Up Marker"),icon: BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueYellow),infoWindow: InfoWindow(title: initialPos.placeName,snippet: "my Location"),
    position: pickUpLatlan,

    );
    Marker dropOffLocationMarker = Marker(markerId: MarkerId("DropOff Marker"),icon: BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueRed),infoWindow: InfoWindow(title: finalPos.placeName,snippet: "DropOff Location"),
    ...
    Circle pickUpCircle = Circle(circleId: CircleId("PickUpID"),
  ...
  void updateAvailableDriversOnMap(){
    ...
    for(NearbyAvailableDrivers driver in GeofireAssistant.nearbyAvailableDriversList){
      ...
      LatLng driverAvailabePosition = LatLng(driver.latitude!, driver.longitude!);
  ...
  void createIconMarker(){
    if(nearbyIcon == null){
      ImageConfiguration imageConfiguration = createLocalImageConfiguration(context,size: Size(2.0, 2.0));
  ...
  void searchNearestDriver(){
    ...
    driversRef.child(driver!.key!).child("car_details").child("type").once().then((value) async {
